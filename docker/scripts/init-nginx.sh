...
for TEMPLATE_NAME in ${PACKAGE_DOCKER_FOLDER_CONTAINER}/nginx/conf-dynamic.d/*.conf; do
    ...
    # root path
    REPLACE_ROOT_TEMPLATE=""
    if [[ -z "${REPLACE_ROOT+x}" ]]; then
        REPLACE_ROOT_TEMPLATE=$(findPattern "root-path" "$BASE_TEMPLATE_CODE}")
    else
      REPLACE_ROOT_TEMPLATE=$REPLACE_ROOT
    fi
    ...
    for DOMAIN in ${ONLY_DOMAINS}; do
        ...
        TEMPLATE_CODE="${TEMPLATE_CODE//\$ROOT_PATH/$REPLACE_ROOT_TEMPLATE}"
